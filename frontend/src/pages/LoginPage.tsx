...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (tempToken) {
        const loggedUser = await verify2FA(
          tempToken,
          twoFactorCode,
      ...
      const loggedUser = result as AuthUser;
      const state = location.state as { from?: { pathname?: string } } | null;
      const redirectTo = state?.from?.pathname || getRedirectPath(loggedUser);
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl sm:text-2xl font-bold font-poppins text-dark mb-2">
              {tempToken ? 'Vérification 2FA' : 'Bienvenue'}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {!tempToken ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    onClick={reset2FAStep}
                    className="text-sm text-slate hover:text-dark"
                  >
              ...
              <Button
                ...
                disabled={submitting}
              >
                {submitting ? 'Connexion...' : tempToken ? 'Vérifier' : 'Se connecter'}
              </Button>
            </form>
          </Card>
